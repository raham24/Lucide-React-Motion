...
- **Motion's `pathLength` shortcut leaves a seam on closed paths.** Setting `pathLength: [0, 1]` in a variant permanently writes `pathLength="1"` + `stroke-dasharray="1 1"` + `stroke-dashoffset="0"` to the rendered element. On open-but-closing paths (gear, cloud, heart, settings) the normalized dash boundary lands on the closure and the two round linecaps render with a visible gap. The default `draw` mode now uses real `stroke-dasharray` + `stroke-dashoffset` driven by `ctx.pathLength` (measured via `getTotalLength()`) and clears both at rest via `transitionEnd` — see `src/modes/draw.ts`. **Mirror that pattern for any NEW draw-in motion you author.** Existing bespoke modifier-reveal motions in `src/modes/motions/` still use Motion's `pathLength` value; they're scheduled for migration (project memory `project_pathlength_motion_migration`). When extending one of those existing modifier-reveal families, keep using Motion's `pathLength` for coupling consistency with the family's neighbours — but for any new standalone draw-in, use the dasharray pattern.
