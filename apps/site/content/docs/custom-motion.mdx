...
The `variants` prop is the low-level escape hatch. Supply your own Motion variants and the engine skips its built-in draw logic, using your variants directly.

## When to reach for this

- You want **whole-icon motion** (pulse, spin, wiggle) rather than stroke-by-stroke draw.
- You want a **bespoke effect** for a specific icon that doesn't fit any preset.
- You want to compose multiple motion properties at once.

For most apps, the built-in `duration` / `delay` / `stagger` / `easing` props are enough. Reach for `variants` only when those aren't.
