...
| `motions/scan-search-loupe.ts` | `scan-search` loupe `<circle cx=12 cy=12 r=3>` + handle `m16 16-1.9-1.9` | Damped magnifier wobble `rotate: [0, -10, 8, -5, 2, 0]` — same rhythm as `searchScan` but pivoted at scan-search's loupe centre (12, 12) instead of the base search's (11, 11) |
