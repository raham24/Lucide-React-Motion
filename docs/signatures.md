...
| `motions/folder-body.ts` | Standard closed folder body (2 spellings), the alternate canonical (`folder-dot`/`-kanban`/`-root`), 13 reshaped bodies where a composite inset-cuts the corner (`-archive`, `-clock`, `-cog`, `-git-2`, `-heart`, `-input`, `-key`, `-lock`, `-output`, `-pen`, `-search`, `-symlink`, `-sync`), and 2 open-folder shapes (`-open`, `-open-dot`) — 18 path d's total | Folder picked-up gesture: `y: [0, -1, 0]` + opacity dim `[1, 0.78, 1]` over `times: [0, 0.4, 1]` — reads as the folder being lifted/selected in a file manager. Sub-icons inherit BOTH the lift and the opacity via the family modifier-reveal so payloads stay anchored. Exports `FOLDER_BODY_KEYFRAMES` |
| `motions/folder-modifier-reveal.ts` | `matchAnyPath` wildcard for every non-body folder-family path: state markers (`-check`, `-x`, `-plus`, `-minus`, `-up`/`-down`), payload glyphs (`-bookmark`, `-code` chevrons, `-kanban` bars, `-clock` arm, `-cog` teeth, `-git` connectors, `-heart`, `-input`/`-output`/`-symlink` arrows + dividers, `-key` shackle, `-lock` shackle, `-pen` body, `-root` post, `-search`/`-search-2` loupe, `-sync` arrows, `-closed` divider), and shape payloads (`-archive`/`-dot`/`-git`/`-git-2`/`-clock`/`-cog`/`-key`/`-open-dot`/`-root`/`-search`/`-search-2` circles, `-lock` rect) | Branches by element type — `<path>` / `<line>` get dasharray draw-in, `<circle>` / `<rect>` get scale-from-0 pivoted at the element's own `fill-box` centre. Reveal completes at `t = 0.4` (the folder's lift apex). Opacity tracks `FOLDER_BODY_KEYFRAMES.opacity` from 0 — `[0, 0, 0.78, 1]`. `y` inherits the body's lift directly. Place last in the compose list |
