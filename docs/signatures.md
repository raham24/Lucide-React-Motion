...
| `motions/rotate-key-turn.ts` | The key body paths (`M12 7v6`, `M12 9h2`) + `<circle cx=12 cy=15 r=2>` in `rotate-ccw-key` | Key turns counter-clockwise around its keyhole pivot (12, 15) by -90° while the arc swings around (12, 12). Implemented via `rotate` + a synthesized `translate` ramp `(3·sin(θ), 3 − 3·cos(θ))` that pins the keyhole at (12, 15) at every angle of the turn (the signature's global `transformOrigin` is at (12, 12), not the keyhole). Shares `ROTATE_ARC_KEYFRAMES.times` with the arc so both apexes land together; skips inheriting the arc's `scale` since the arc winds-and-spins while the key turns — two simultaneous gestures with different roles (Tier 2) |
