...
| `motions/scan-corners-modifier-reveal.ts` | `matchAnyPath` wildcard for every interior payload in the scan-corners family: `focus` reticle circle, `fullscreen` inner rect, `scan-line` sweep line, `scan-barcode` vertical bars, `scan-eye` pupil + eyelid arc, `scan-face` smile + cheek dots, `scan-heart` heart, `scan-qr-code` QR fragments + rect, `scan-search` magnifier + handle, `scan-text` text lines | Branches by element type — `<path>` / `<line>` get dasharray draw-in completing at `t = 0.2` (the brackets' lock-on apex), `<circle>` / `<rect>` get scale-from-0 around the signature's (12, 12) pivot (no fill-box override — every payload's centre is within ~1 unit of icon centre, so the inward bloom reads as the scanner focusing on the subject). Opacity tracks `SCAN_CORNERS_KEYFRAMES.opacity` from 0 — `[0, 0, 0.85, 0.85, 1]`. Scale inherits the bracket contraction directly. Place last in the compose list |
