...
| `motions/search-scan.ts` | `matchAnyPath` (wildcard) for every `search` variant — loupe `<circle cx=11 cy=11 r=8>`, handle `m21 21-4.3…-4.3…`, and any in-loupe markers (alert stem + dot, check, slash, ×, code brackets) | Damped side-to-side rotation wobble around the loupe centre at (11, 11) — reads as a magnifying glass tilting to inspect a target. Signature pivots at `"11px 11px"` so rotation operates around the loupe rather than the icon centre (the handle's far tip at (21, 21) at radius ≈ 14.14 stays inside the 24×24 viewBox at ±10° peaks). In-loupe markers and the code brackets share the rotation directly via `matchAnyPath` — rotation is in-plane so diagonal strokes don't axis-asymmetrically flatten (Tier 2 — magnifier scan per section 7) |
