...
| `motions/wifi-sync-arrows.ts` | The four sync-arrow paths in `wifi-sync` | The paired sync arrows rotate in lockstep around the badge center while drawing in, so the sync control moves as an actual rotating loop rather than a static overlay. |
