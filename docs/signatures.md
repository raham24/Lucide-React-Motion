...
| `motions/message-square-modifier-reveal.ts` | `matchAnyPath` wildcard for every non-body message-square-family path: state markers (`-check`, `-x`, `-plus`, `-diff`, `-warning`, `-off` slash, `-more` ellipsis, `-text` lines), payload glyphs (`-heart`, `-code` chevrons, `-reply` / `-share` arrows, `-quote` marks, `-lock` shackle), and shape payloads (`-dot` circle, `-lock` rect) | Branches by element type — `<path>` / `<line>` get dasharray draw-in, `<circle>` / `<rect>` get scale-from-0 pivoted at the element's own `fill-box` centre. Reveal completes at `t = 0.25` (the body's first nod apex). Opacity tracks `MESSAGE_SQUARE_BODY_KEYFRAMES.opacity` from 0 — `[0, 0, 0.78, 1, 0.9, 1]`. Rotate inherits the body's nod directly. Place last in the compose list |
