...
| `motions/cloud-lightning-bolt.ts` | `cloud-lightning`'s bolt d | Translate descent (`y: [-5, 0, ...]`) from inside the cloud body to rest position over 30% of the cycle, with opacity ramping to 1 at t=0.15 so the second half of the fall is fully visible (avoids the glitchy teleport-in feel of a too-fast descent). After landing, multi-flash opacity flicker (1 → 0.15 → 1 → 0.4 → 1). Inherits cloudBody scale for cohesion (Tier 2) |
...
| `motions/zap-strike.ts` | Full zap bolt d + the three fragments in `zap-off` | Translate descent (`y: [-6, 0, ...]`) over 40% of the cycle with easeIn (gravity-like acceleration). Distance is deliberately short and time deliberately long — the previous version's 24-unit descent in 80 ms read as a glitch because the eye couldn't track the motion. Opacity reaches 1 at t=0.20 (half-way through the descent) so the second half of the fall is fully visible. After landing, multi-flash flicker (1 → 0.15 → 1 → 0.4 → 1) with linear easing for crisp peaks. Exports `ZAP_KEYFRAMES` (Tier 2 — bespoke lightning physics: descent + flash) |
| `motions/zap-modifier-reveal.ts` | `matchAnyPath` (wildcard) | pathLength + opacity reveal for `zap-off`'s diagonal slash, timed to complete at the host's first dim trough (`t = 0.50`, between the primary flash and the secondary peak) so the slash lands in the dark moment right after the strike. Synthesized uniform `scale` dip `[1, 0.92, 1]` provides continuous kinetic life through the rest of the cycle. Skips host's `y` inheritance (slash stays at rest position; sliding down would destroy the strike-through reading) |
