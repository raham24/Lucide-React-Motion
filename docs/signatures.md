...
| `motions/medal-ribbon-sway.ts` | The 4 `medal` neck-ribbon strap d's (band + 2 fold lines + stripe) via `matchPathDOneOf` | Anchorless horizontal `x: [0, -1.6, 1, -0.4, 0]` sway (same mechanism as `awardRibbonSway`). The cloth strap sways while the medallion disc + "1" numeral fall through to `polishedShineSweep` and catch light in place. Placed FIRST in `medal`'s compose list — the same `[ribbonSway, polishedShineSweep]` shape as `award` |
