...
| `motions/wifi-sync-arrows.ts` | The four sync-arrow paths in `wifi-sync` | Simple loader-style spin for the sync strokes: each path rotates around its own fill-box center (`transformBox: fill-box`, `transformOrigin: center`) instead of orbiting around a shared badge center. |
...
| `motions/rotate-key-turn.ts` | The key body paths (`M12 7v6`, `M12 9h2`) + `<circle cx=12 cy=15 r=2>` in `rotate-ccw-key` | Key spins one full counter-clockwise revolution around its keyhole pivot (12, 15) while the arc swings around (12, 12) — the "spin the key in the lock" gesture. Implemented via `rotate` + a synthesized `translate` ramp `(3·sin(θ), 3 − 3·cos(θ))` sampled every 90° so the keyhole stays pinned at (12, 15) through the spin (the signature's global `transformOrigin` is at (12, 12), not the keyhole; without the translate compensation the keyhole would orbit around the icon centre). The spin's −180° midpoint is aligned to the arc's apex (t = 0.5) so the two gestures share one synchronized moment, and the spin completes at t = 0.85 ending at −360 ≡ 0 visually (same end-of-active pattern as `refreshArcCycle` holding at 360). Skips inheriting the arc's `scale` — the arc winds-and-spins, the key spins (Tier 2) |
