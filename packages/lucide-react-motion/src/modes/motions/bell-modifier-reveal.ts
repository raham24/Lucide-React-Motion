...
/**
 * Bell-family wildcard reveal — `modifierReveal`'s draw-in (pathLength
 * + opacity) plus the host `bellShell` rotation so every non-shell
 * non-clapper path in a bell variant rocks *with* the bell instead of
 * floating statically over a swinging icon.
 *
...
export const bellModifierReveal: Motion = {
  ...
  factory: (ctx) => ({
    rest: { pathLength: 1, opacity: 1, rotate: 0 },
    active: {
      pathLength: [0, 0, 1],
      opacity: [0, 0, 1],
